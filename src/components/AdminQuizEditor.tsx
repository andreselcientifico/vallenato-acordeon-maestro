      ...
      prev.map((q, i) =>
        i === qIndex
          ? {
              ...
              options: [
                ...q.options,
                { text: "", is_correct: false, order: q.options.length + 1 },
              ],
            }
          : q,
      ),
        ...
        const options = q.options.map((opt, oi) =>
          oi === optIndex
            ...
            : {
                ...opt,
                is_correct:
                  field === "is_correct" && value ? false : opt.is_correct,
              },
        );
  ...
  async function handleSave() {
    ...
    for (const q of questions) {
      ...
      if (!q.options.some((o) => o.is_correct)) {
        alert("Cada pregunta necesita una opción correcta");
        return;
      }
    }
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <Input
                      value={q.question}
                      onChange={(e) =>
                        updateQuestionField(qi, "question", e.target.value)
                      }
                    ...
                    <Textarea
                      value={q.description || ""}
                      onChange={(e) =>
                        updateQuestionField(qi, "description", e.target.value)
                      }
                      placeholder="Descripción (opcional)"
                      className="mb-2"
                    />
                    <Textarea
                      value={q.explanation || ""}
                      onChange={(e) =>
                        updateQuestionField(qi, "explanation", e.target.value)
                      }
                      ...
                      {q.options.map((opt, oi) => (
                        <div key={oi} className="flex gap-2 items-center">
                          <input
                            ...
                            onChange={() =>
                              updateOption(qi, oi, "is_correct", true)
                            }
                            className="mt-1"
